...
void USART2_IRQHandler(void)
{
  /* USER CODE BEGIN USART2_IRQn 0 */
   ...
   {   
		//USART2->ISR &= ~(USART_ISR_RXNE_RXFNE);  //auto cleared by readed
      
   //flg_rx=1;
      
      cuartbyte = (uint8_t)(READ_BIT(USART2->RDR, USART_RDR_RDR) & 0xFFU); 
   ...
   {
      __IO uint32_t iregval;
      iregval = LL_USART_ReadReg(USART2, ISR);
      ...
      LL_USART_ClearFlag_RTO(USART2);   //11
      LL_USART_ClearFlag_UDR(USART2);   //13
